...
const CustomTimeline = ({ tasks, onEdit, onDelete, newTaskCount, updatedTaskCount }) => {
  ...
  // Calculate line heights after DOM updates
  useLayoutEffect(() => {
    const calculateLineHeights = () => {
      ...
      sortedTasksWithAds.forEach((item, index) => {
        ...
        if (currentDot && nextDot) {
          ...
          const distance = nextDotCenter - currentDotCenter;
          heights[item.id] = distance; // Line should span from dot center to dot center
        } else {
          heights[item.id] = expandedTasks[item.id] ? 180 : 80; // Adjusted fallback
        }
      });
      setLineHeights(heights);
    };

    // Run initially and after expansion changes
    calculateLineHeights();
    const timeout = setTimeout(calculateLineHeights, 250); // Account for animation
  ...
  return (
    <div className="flex-1 relative overflow-hidden w-full">
      ...
      <div
        ref={scrollContainerRef}
        className="max-h-[calc(100vh-180px)] overflow-y-auto px-4 py-2 space-y-4 w-full"
        onScroll={handleScroll}
      >
        {sortedTasksWithAds.map((item, index) => {
          if (item.isAd) {
          ...
          const textColor = isLightColor(backgroundColor) ? '#1f2937' : '#f9fafb';
          const dotColor = isCompleted ? '#3b82f6' : isOverdue ? '#d1d5db' : '#6b7280';
          const innerDotColor = isCompleted ? '#60a5fa' : isOverdue ? '#d1d5db' : '#ffffff';
          const timelineColor = isCompleted ? '#60a5fa' : isOverdue ? '#d1d5db' : '#6b7280';
          const cardBorderColor = isOverdue ? '#d1d5db' : '#e5e7eb';

          return (
            <div key={item.id} id={`task-${item.id}`} className="flex items-start space-x-0 w-full">
  {/* Timeline + Time */}
  <div className="flex flex-col items-center min-w-[80px] sm:min-w-[80px]">
    <div className="relative flex flex-col items-center">
      ...
      <div
        ...
        className="relative w-4 h-4 rounded-full mt-1 ml-5"
        style={{ backgroundColor: dotColor }}
      >
        <div
      ...
      {index !== sortedTasksWithAds.length - 1 && (
        <div
          ...
          style={{ height: lineHeights[item.id] || 80, backgroundColor: timelineColor }}
        />
      )}
    </div>
  </div>
  ...
  <motion.div
    ref={(el) => (taskRefs.current[item.id] = el)}
    className="bg-white border rounded-[5px]  overflow-hidden shadow-sm p-3 transition-all duration-300 flex-1 w-full"
  ...
  >
    ...
    >
      <div className="text-left flex-1 pr-2 overflow-hidden ">
        <h3
          className={`text-sm font-semibold ${isExpanded ? 'break-words' : 'truncate'}`}
          style={{ color: textColor }}
        >
          {item.title}
        </h3>
        {item.description && (
          <p className={`text-xs text-gray-600 mt-1 ${isExpanded ? 'break-words' : 'truncate'}`}>
            {item.description}
          </p>
        )}
      </div>
    ...
    <AnimatePresence>
      {isExpanded && (
        ...
        >
          ...
          {item.note && (
            <div className="flex items-start space-x-2 text-gray-600">
              <FaNoteSticky size={14} className="mt-0.5 flex-shrink-0" />
      ...
      <button
        onClick={scrollToBottom}
        className="absolute bottom-16 right-4 bg-gray-300 hover:bg-gray-400 p-2 rounded-full shadow-md"
      >
