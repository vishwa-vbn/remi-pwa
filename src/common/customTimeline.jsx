...
//       <button
//         onClick={scrollToBottom}
//         className="absolute bottom-4 right-2 bg-gray-300 hover:bg-gray-400 p-2 rounded-full shadow-md"
//       >
...
const sanitizeTask = (task) => ({
  ...task,
  title: task.title?.replace(/</g, "&lt;").replace(/>/g, "&gt;"),
  description: task.description?.replace(/</g, "&lt;").replace(/>/g, "&gt;"),
  note: task.note?.replace(/</g, "&lt;").replace(/>/g, "&gt;"),
});
...
const CustomTimeline = ({
  ...
  // Memoized sorted tasks with ads
  const sortedTasksWithAds = useMemo(() => {
    const sorted = [...tasks]
      .map(sanitizeTask)
  ...
  const toggleExpanded = useCallback((taskId) => {
    const now = Date.now();
    const adInterval = 15 * 60 * 1000; // 15 minutes
    if (!lastAdTime || now - lastAdTime >= adInterval) {
      setShowRandomAd(true);
      setLastAdTime(now);
      setTimeout(() => setShowRandomAd(false), 5000);
    }
    setExpandedTasks((prev) => ({
      ...prev,
      [taskId]: !prev[taskId],
  ...
  useEffect(() => {
    ...
    if (newTaskIndex !== -1 && !sortedTasksWithAds[newTaskIndex].isAd) {
      const taskElement = document.getElementById(
        `task-${sortedTasksWithAds[newTaskIndex].id}`
      );
      if (taskElement) {
        taskElement.scrollIntoView({ behavior: "smooth", block: "center" });
        setHighlightedTask(sortedTasksWithAds[newTaskIndex].id);
        const timeout = setTimeout(() => setHighlightedTask(null), 600);
  ...
  return (
    <div className="flex-1 bg-white relative h-full overflow-hidden w-full">
      ...
      <div
        ref={scrollContainerRef}
        className="max-h-[calc(100vh-180px)] overflow-y-auto px-3 py-2 space-y-2 w-full scrollbar-hide"
        onScroll={handleScroll}
      >
        {sortedTasksWithAds.map((item, index) => {
          if (item.isAd) {
          ...
          const isCompleted = item?.status === true;
          const currentTime = Date.now();
          const isOverdue = !isCompleted && (item.timestamp || 0) < currentTime;
          const backgroundColor = isOverdue
            ? "#f8f9fa"
            : getFadedColor(item.title || "");
          const textColor = isLightColor(backgroundColor)
  ...
  >
    <FaClock size={14} color={iconColor} />
    <span>
      Alert: {item.alertMinutes === 0 ? "at " : `${item.alertMinutes} min before at `}
      {new Date(
        item.alertMinutes === 0 ? item.timestamp : item.timestamp - item.alertMinutes * 60000
      ).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
        hour12: true,
